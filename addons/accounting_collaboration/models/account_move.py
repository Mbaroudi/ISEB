...
class AccountMove(models.Model):
    """
    Extension du modèle de pièce comptable pour ajouter les questions
    """
    ...
    has_pending_questions = fields.Boolean(
        ...
        store=True,
        help="Indique si cette pièce a des questions en attente de réponse"
    )
    ...
    def _compute_question_count(self):
        ...
        for move in self:
            move.question_count = len(move.question_ids)

    @api.depends('question_ids.state')
    def _compute_has_pending_questions(self):
        """Vérifie si des questions sont en attente"""
        for move in self:
            move.has_pending_questions = bool(
                move.question_ids.filtered(lambda q: q.state in ['pending', 'answered'])
            )

    def action_create_question(self):
        """Ouvre un formulaire pour créer une question sur cette pièce"""
        self.ensure_one()
    ...
    def action_view_questions(self):
        """Affiche les questions liées à cette pièce"""
        self.ensure_one()
