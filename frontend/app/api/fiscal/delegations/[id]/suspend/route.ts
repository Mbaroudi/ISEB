...
export async function POST(
  ...
  try {
    ...
    const readResponse = await axios.post(
      `${odooUrl}/web/dataset/call_kw`,
      {
        ...
        params: {
          ...
          args: [[parseInt(id)]],
          kwargs: {
            fields: ["name", "partner_id", "state", "is_active"],
          },
        },
      },
      {
    ...
    return NextResponse.json({
      success: true,
      delegation,
      message: "Délégation suspendue avec succès",
    });
  } catch (error: any) {
    console.error("Suspend delegation error:", error);

    let errorMessage = "Failed to suspend delegation";
