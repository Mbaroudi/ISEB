...
export default function DocumentsPage() {
  ...
  const applyOCR = useApplyOCR();
  const { toast } = useToast();

  // Load tags and categories with React Query
  ...
  const handleExtractOCR = async (docId: number) => {
    ...
    try {
      ...
      toast({ title: "OCR extrait", description: `Données extraites avec ${data.confidence_score}% de confiance.` });
    } catch (error: any) {
      console.error("Error extracting OCR:", error);
      toast({ title: "Erreur OCR", description: "Impossible d'extraire les données.", variant: "destructive" });
    } finally {
      setOcrLoading(null);
    }
  };
  ...
  const handleApplyOCR = async (docId: number) => {
    try {
      ...
      toast({ title: "Données appliquées", description: "Les données OCR ont été appliquées au document." });
      setShowOcrResult(false);
      searchDocuments(); // Refresh
    } catch (error: any) {
      console.error("Error applying OCR:", error);
      toast({ title: "Erreur", description: "Impossible d'appliquer les données OCR.", variant: "destructive" });
    }
  };
