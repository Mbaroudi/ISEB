...
import { useImportAccounting, useExportAccounting } from "@/lib/hooks/useSettings";
import {
  User,
  Building2,
  Bell,
...
export default function SettingsPage() {
  ...
  const handleImport = async () => {
    if (!importFile) {
      toast({ title: "Fichier requis", description: "Veuillez sélectionner un fichier.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const result = await importAccounting.mutateAsync(formData);
      setImportResult(result);

      if (result.success) {
        toast({ title: "Import réussi", description: `${result.successCount} écritures importées.` });
      }
    } catch (error: any) {
      setImportResult({
        success: false,
  ...
  const handleExport = async () => {
    if (!exportDateFrom || !exportDateTo) {
      toast({ title: "Période requise", description: "Veuillez sélectionner une période.", variant: "destructive" });
      return;
    }

    try {
      const result = await exportAccounting.mutateAsync({
      ...
      if (result.success) {
        ...
        toast({ title: "Export réussi", description: "Le(s) fichier(s) ont été téléchargé(s)." });
      } else {
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-6 space-y-4">
                    ...
                    <Button
                      ...
                      disabled={!importFile || importAccounting.isPending}
                      className="w-full"
                    >
                      {importAccounting.isPending ? (
                        <>
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-6 space-y-4">
                    ...
                    <Button
                      ...
                      disabled={!exportDateFrom || !exportDateTo || exportAccounting.isPending}
                      className="w-full"
                    >
                      {exportAccounting.isPending ? (
                        <>
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                          Export en cours...
                        </>
